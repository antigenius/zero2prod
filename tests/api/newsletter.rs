...
use crate::helpers::{assert_is_redirected_to, ConfirmationLinks, spawn_app, TestApp};
...
#[tokio::test]
async fn concurrent_submission_handled_gracefully() {
    let app = spawn_app().await;
    create_confirmed_subscriber(&app).await;
    app.test_user.login(&app).await;
    ...
    let response1 = app.post_newsletter(&body);
    let response2 = app.post_newsletter(&body);
